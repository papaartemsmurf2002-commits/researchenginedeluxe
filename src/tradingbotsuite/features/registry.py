...
def feature_set_presets() -> dict[str, tuple[str, ...]]:
    return {
        ...
        "features_perp_context_v3": ("perp_context_v3",),
        "features_aggtrade_orderflow_v1": ("aggtrade_orderflow_v1",),
        ...
        "features_cross_asset_btc_eth_v2": ("cross_asset_btc_eth_v2",),
...
CROSS_ASSET_BTC_ETH_V2_COLUMNS = (
    ...
    "quality_cross_asset_missing_btc_context",
    "quality_cross_asset_missing_eth_context",
    "quality_cross_asset_missing_context",
    "quality_cross_asset_missing_durable_context",
    "quality_cross_asset_missing_funding_context",
    "quality_cross_asset_missing_oi_context",
    "quality_cross_asset_future_alignment_risk",
    "quality_cross_asset_matched_interval",
    "quality_cross_asset_point_in_time_join",
    "quality_cross_asset_candidate_ready_eligible",
)
CALENDAR_COLUMNS = (
    "session_hour_sin",
    "session_hour_cos",
...
_FEATURE_PACKS = (
    ...
    FeaturePack(
        pack_id="perp_context_v3",
        version="v1",
        input_families=("funding_rate", "premium_index", "open_interest", "agg_trade"),
        feature_specs=_specs(PERP_CONTEXT_V3_COLUMNS, "As-of perpetual context v3 with source eligibility"),
        point_in_time_safe=True,
        fit_scope="stateless",
        imputation_policy="explicit_missingness",
        optional=True,
        leakage_risks=(
            "Latest-window derivatives context is diagnostic and not candidate-ready evidence.",
            "AggTrade fields are trade-flow proxies, not order-book imbalance or true OFI.",
        ),
    ),
    FeaturePack(
        pack_id="aggtrade_orderflow_v1",
        version="v1",
        input_families=("agg_trade",),
        feature_specs=_specs(AGGTRADE_ORDERFLOW_COLUMNS, "AggTrade trade-flow proxy"),
        point_in_time_safe=True,
        fit_scope="stateless",
        imputation_policy="explicit_missingness",
        optional=True,
        leakage_risks=(
            "AggTrade orderflow features are trade-flow proxies, not true OFI or order-book imbalance.",
            "Latest-window or unknown aggTrade context is diagnostic until source readiness proves durability.",
        ),
    ),
    FeaturePack(
    ...
    FeaturePack(
        pack_id="cross_asset_btc_eth_v2",
        version="v2",
        input_families=("kline", "funding_rate", "open_interest"),
        feature_specs=_specs(CROSS_ASSET_BTC_ETH_V2_COLUMNS, "Point-in-time BTC/ETH residual context"),
        point_in_time_safe=True,
        fit_scope="stateless",
        imputation_policy="explicit_missingness",
        optional=True,
        leakage_risks=(
            "Cross-symbol BTC/ETH joins require source timestamps at or before the current completed bar.",
            "Missing durable BTC or ETH context blocks candidate readiness and must remain explicit, not zero-filled.",
        ),
    ),
    FeaturePack(
        pack_id="calendar_v1",
