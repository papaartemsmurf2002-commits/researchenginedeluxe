...
class ResearchUiService:
    ...
    def _execute_job(self, job: ResearchJob) -> None:
        ...
        job.status = "running"
        try:
            result = run_research_experiment(spec_path=Path(job.spec_path), app_config=AppConfig.from_env())
            job.result = {
...
def create_research_app(config: AppConfig | None = None, service: ResearchUiService | None = None) -> FastAPI:
    ...
    service = service or ResearchUiService(research_root=config.research.output_dir)
