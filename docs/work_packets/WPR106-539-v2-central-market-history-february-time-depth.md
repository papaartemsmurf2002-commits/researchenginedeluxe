# WPR106-539 - V2 Central Market History February Time Depth
...
cap. This packet extends time depth for high-liquidity symbols that already
have January 2024 central-store coverage.
...
At packet start the central store contains 2335 files and 17861318299 bytes
(approximately 16.635 GiB), leaving approximately 133.365 GiB under the 150 GiB
cap. The append manifest has 31 prior rows from WPR106-534 through WPR106-538.

The current worktree also contains prior uncommitted WPR106-527 through
WPR106-538 changes. They are treated as authoritative and must not be reverted
or rewritten outside this packet's scope.

## Allowed Paths

- `docs/work_packets/WPR106-539-v2-central-market-history-february-time-depth.md`
...
- Collect public official Binance USD-M aggregate-trade ZIPs and Bybit public
  linear trading GZIPs for February 2024 BTC, ETH, SOL, BNB, XRP, and DOGE
  where reachable.
...
WPR106-539 kept all accepted generated artifacts under
`data/research/central_market_history/**`. Final measured central root size is
20818994790 bytes, approximately 19.389 GiB, leaving approximately 130.611 GiB
under the 150 GiB cap. The append manifest now has 37 rows.

WPR106-539 appended 870,000 normalized rows through six additional central
batches:

- `wpr106-539-btc-feb2024-trade-orderflow`: 145,000 BTC rows from 29 Binance
...
- `wpr106-539-eth-feb2024-trade-orderflow`: 145,000 ETH rows from 29 Binance
  USD-M aggregate-trade ZIPs; 29 Bybit public trading paths returned 404 and
  are recorded as provider-specific source blockers.
- `wpr106-539-sol-feb2024-trade-orderflow`: 145,000 SOL rows from 29 Binance
  USD-M aggregate-trade ZIPs; 29 Bybit public trading paths returned 404 and
  are recorded as provider-specific source blockers.
- `wpr106-539-bnb-feb2024-trade-orderflow`: 145,000 BNB rows from 29 Binance
  USD-M aggregate-trade ZIPs; 29 Bybit public trading paths returned 404 and
  are recorded as provider-specific source blockers.
- `wpr106-539-xrp-feb2024-trade-orderflow`: 145,000 XRP rows from 29 Binance
  USD-M aggregate-trade ZIPs; 29 Bybit public trading paths returned 404 and
  are recorded as provider-specific source blockers.
- `wpr106-539-doge-feb2024-trade-orderflow`: 145,000 DOGE rows from 29 Binance
  USD-M aggregate-trade ZIPs; 29 Bybit public trading paths returned 404 and
  are recorded as provider-specific source blockers.

All 174 Binance USD-M official public archives downloaded, parsed, and
manifested. The 174 Bybit public trading source probes for these symbols and
...
The aggregate normalized central Parquet set now contains 6,028,343 rows:

- Providers: `binance_usdm=3745980`, `bybit_linear=1596913`,
  `binance_spot=352384`, `bybit_inverse=178560`, `hyperliquid=144506`,
  `bybit_spot=10000`.
- Families: `orderflow=3500999`, `ohlcv=1412324`, `trade=936000`,
  `metadata=179020`.
- Timeframes: event/metadata rows without a row timeframe at `4616019`,
  `1h=1251630`, `15m=116927`, and `1d=43767`.

This packet extends time depth for January-covered high-liquidity symbols into
February 2024. BTC and ETH remain fixture/reference symbols, not the full v2
product scope.
...
active WPR106-539 collection processes remained after collection.
