...
Audit IDs: `V2-AUD-AUTONOMY-001`, `V2-AUD-AUTONOMY-004`, `V2-AUD-AUTONOMY-006`, `V2-AUD-AUTONOMY-007`, `V2-AUD-AUTONOMY-008`, `V2-AUD-AUTONOMY-009`, `V2-AUD-AUTONOMY-010`, `V2-AUD-AUTONOMY-011`, `V2-AUD-AUTONOMY-012`, `V2-AUD-AUTONOMY-013`, `V2-AUD-AUTONOMY-014`, `V2-AUD-AUTONOMY-017`, `V2-AUD-AUTONOMY-018`
...
The durable `backtest_data_load` worker is the bounded loop's explicit data
loading stage. It may load archive-backed panels only through
`BacktestDataService`, write canonical backtest-data manifest evidence, and
return archive, universe, coverage, and data-manifest refs for the later
`vectorized_backtest` worker to verify before running.
...
least one collector/archive stage, coverage audit, strategy queue scan,
backtest-data load, vectorized backtest, validation gate, ledger append/export,
and Lead Book upsert, in that order. It must reject user-supplied
`audit_check` jobs because the final audit job is generated by the planner from
the selected target job IDs and required loop evidence.
...
4. `strategy_queue_scan` over the generated declarative fixture spec file.
5. `backtest_data_load` bound to the generated universe and archive snapshots.
6. `vectorized_backtest` bound to the backtest-data archive, universe,
   coverage, and data-manifest refs plus the accepted strategy spec path and
   SHA-256.
7. `validation_gate` bound to the backtest run manifest.
8. `ledger_append_export` bound to the backtest run manifest and validation
   manifest path.
9. `lead_book_upsert` bound to the canonical ledger path.
10. The planner-generated final `audit_check` job.
...
expected to finish as `completed_with_blockers` until authoritative
free-venue data evidence, readiness artifacts, and independent completion
audit evidence exist.

## Existing Archive-Ref Cycle Spec

`redx autopilot archive-cycle-spec` may write a canonical
`autopilot_bounded_cycle_spec_v1` for already-built local archive evidence. The
command requires an existing archive root, existing strategy-spec directory,
`archive_snapshot_id`, and `universe_snapshot_id`; it writes only the cycle
spec and run directories, and it does not collect provider data, write raw,
bronze, silver, or gold market-data rows, enqueue jobs, or run workers.

The generated archive-ref cycle declares:

1. `universe_refresh` with `source=existing_ref`.
2. `recent_candle_bootstrap` with `source=existing_ref`.
3. `coverage_audit` bound to the verified universe and archive refs.
4. `strategy_queue_scan` over the supplied declarative JSON/YAML spec
   directory.
5. `backtest_data_load` bound to the verified universe and archive refs.
6. `vectorized_backtest` bound to the backtest-data archive, universe,
   coverage, and data-manifest refs plus the accepted strategy spec path and
   SHA-256.
7. `validation_gate` bound to the backtest run manifest.
...
Archive-ref cycle specs are accepted-research-cycle execution plans only when
the supplied refs and uploaded strategy specs pass all downstream gates. A
cycle may finish with a passing final audit and no blockers, but the audit
report and execution manifest must still keep `accepted_research_ready=false`
and all promotion, candidate, paper/live, sizing, order, and runtime flags
false.
...
- Bounded cycle plans must require an explicit `backtest_data_load` stage
  between `strategy_queue_scan` and `vectorized_backtest`, and the generated
  audit job must require backtest-data manifest and data-manifest refs.
...
- Producing candidate packs, paper signals, live signals, sizing instructions,
  ...
  artifacts.
- Suppressing blockers for missing authoritative free-venue archive/coverage
  evidence.
...
- Treating a bounded cycle execution manifest or passing generated audit report
  as autonomous-ready release proof without authoritative free-venue
  archive/coverage evidence, independent audits, authoritative validation, and
  open-blocker closure.
- Treating a bounded scheduler tick as a daemon, live runtime, venue fetch
  ...
  promotion evidence.
- Treating the executable fixture cycle as authoritative free-venue archive
  operation, accepted strategy evidence, autonomous-ready certification, or a
  replacement for independent completion audit.
...
final generated audit report with explicit missing-readiness-evidence
blockers. It is never accepted as autonomous-ready or strategy performance
evidence.
...
strategy queue scan, backtest-data load, vectorized backtest, validation gate, ledger
append/export, Lead Book upsert, and the planner-generated final `audit_check`
job.
