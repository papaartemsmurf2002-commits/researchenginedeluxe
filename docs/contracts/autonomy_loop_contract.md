...
vectorized backtest, validation gate, ledger append/export, and Lead Book
upsert, in that order. It must reject user-supplied `audit_check` jobs because
the final audit job is generated by the planner from the selected target job
IDs and required loop evidence.
...
5. `vectorized_backtest` bound to the generated universe and archive
   snapshots plus the accepted strategy spec path and SHA-256.
6. `validation_gate` bound to the backtest run manifest.
7. `ledger_append_export` bound to the backtest run manifest and validation
   manifest path.
8. `lead_book_upsert` bound to the canonical ledger path.
9. The planner-generated final `audit_check` job.
...
strategy queue scan, vectorized backtest, validation gate, ledger
