# Research Knowledge Catalog

This directory stores non-authoritative research knowledge that can help future
agents design falsifiable experiments.

These files are not implementation roadmaps, candidate evidence, promotion
evidence, or live trading instructions. Treat them as hypothesis maps and
backtest-design references. Every idea still needs repo-native data manifests,
point-in-time features, cost/funding/slippage accounting, split-safe
validation, ablation evidence, and research gates before it can influence any
candidate decision.

## Catalog

| Artifact | Source | Use |
| --- | --- | --- |
| `BTC_ETH_PERP_STRATEGY_KNOWLEDGE_BASE.md` | External master report from `C:/Users/papaa/Downloads/btc_eth_perp_strategies_master_report.md` | BTC/ETH perpetual strategy taxonomy, feature/data requirements, simulator standards, and red-team cautions for future testing design. |
| `source_reports/btc_eth_perp_strategies_master_report.md` | Imported verbatim from `C:/Users/papaa/Downloads/btc_eth_perp_strategies_master_report.md` | Full detail reference when the catalog is too condensed; still hypothesis material only. |
